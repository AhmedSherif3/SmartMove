...
*   **Logic Explanation**: Groups lists dynamically into monthly digests or yearly summaries. It applies multi-select constraints matching country/region select elements, month dropdowns, date days, and year properties.
...
```tsx
  ...
  const handlePreview = (report: ApiReportItem) => {
    const previewEndpoint = `${getApiBaseUrl()}/reports/${report.id}/preview/`;
...
### PdfPreviewModal.tsx
*   **Logic Explanation**: Integrates custom `<canvas>` elements rendering PDFs page-by-page via PDF.js. It streams the PDF through the secure Django preview proxy endpoint (`/api/reports/{id}/preview/`) with `withCredentials: true` to bypass Azure Blob Storage CORS restrictions.
...
*   **Logic Explanation**: Saves and fetches the authenticated JWT session data structure in `localStorage`, appending a 7-day sliding expiry threshold and automatically clearing state if tokens expire.
...
Covers crawlability configurations, sitemaps, structured JSON-LD schemas, and dynamic metadata mappings implemented for the public guest portal.
...
export default function robots(): MetadataRoute.Robots {
  const baseUrl = "https://smartmoveanalytics.me";

  return {
    rules: [
      {
        userAgent: "*",
        allow: ["/", "/about", "/pricing", "/contact", "/help", "/privacy", "/terms", "/cookies"],
        disallow: ["/admin/", "/analyst/", "/user/", "/authentication/"],
      },
    ],
...
```
*   **Logic Explanation**: Programmatically constructs crawl rules. Directs search engines to index the guest layout routes while explicitly protecting administrative and private workspaces (`/admin`, `/analyst`, `/user`, and `/authentication`) from crawling.
...
```
*   **Logic Explanation**: Generates the XML sitemap dynamic indexing path (`sitemap.xml`) to prioritize central features and landing pages over standard policy frameworks.
...
```
*   **Logic Explanation**: Injects structured schemas directly inside the base document body. Declares the Organization schema (linking corporate profiles), WebSite schema (to enable sitelinks search features), and a SoftwareApplication schema detailing the brand's five distinct intelligence tools: MoveIQ, EstateMind, Analytics Pro Engine, Multi-Market Intelligence, and Predictive Engine.
