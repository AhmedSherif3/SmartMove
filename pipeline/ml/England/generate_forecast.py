...
PRICE_MODEL_PATH = ARTIFACTS_DIR / "england_price_model.pkl"
RENT_MODEL_PATH = ARTIFACTS_DIR / "england_rent_model.pkl"
...
def generate() -> pd.DataFrame:
    if not PRICE_MODEL_PATH.exists() or not RENT_MODEL_PATH.exists():
        raise FileNotFoundError("England models not found in artifacts directory.")

    price_model = joblib.load(PRICE_MODEL_PATH)
    rent_model = joblib.load(RENT_MODEL_PATH)
    logger.info("England models loaded successfully.")
    ...
    logger.info(
        "10-Year Projected ROI: %.2f%% (Capital Appreciation: %.0f GBP, Cumulative Rent: %.0f GBP)",
    ...
    if azure_conn_str:
        ...
        logger.info("Pushing 120-month forecast to fact_predictions_england...")
        combined.to_sql('fact_predictions_england', con=engine, if_exists='append', index=False)
        logger.info("Database insertion complete.")
    else:
        output_path = ARTIFACTS_DIR / "england_forecast_120m.csv"
