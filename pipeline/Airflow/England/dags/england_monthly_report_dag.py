# ============================================================
# dags/england_monthly_report_dag.py
#
# DAG: england_monthly_report
#
# Schedule: 0 0 1 * * (midnight on the 1st of every month)
#
# Flow:
#   fetch_market_data          → Dummy SQL fetch (England market KPIs)
#       │
...
_AIRFLOW_ROOT = Path(__file__).resolve().parents[2]  # e.g. pipeline/Airflow/England/
...
def _fetch_market_data(**context):
    ...
    try:
        ...
        query = f"""
            ...
            FROM dbo.fact_transactions_england f
            JOIN dbo.dim_date_england d ON f.date_id = d.date_id
            JOIN dbo.dim_area_england a ON f.area_id = a.area_id
            JOIN dbo.dim_property_england p ON f.property_type_id = p.property_type_id
    ...
    smartmove_index = 61.7  # Retain dummy composite score for now
        ...
        "market_trends": [
            {
                ...
                "magnitude": "Prices flat MoM",
            },
            {
                "name": "BoE Base Rate Impact",
                "direction": "Cooling",
                "magnitude": "Mortgage approvals down 3.2% QoQ",
            },
            {
                "name": "Build-to-Rent (BTR)",
                "direction": "Increasing",
                "magnitude": "Investment up 15% YoY",
            },
            {
                "name": "Energy Efficient Homes",
                "direction": "Premium Growing",
                "magnitude": "EPC A/B homes command 8% premium",
            },
        ],
        "ai_recommendations": [
            "Monitor central London prime properties for foreign investment opportunities.",
            "Watch BoE mortgage rates — potential rate cuts could stimulate Q3 demand.",
            "Consider BTR sector in Northern cities (e.g., Manchester, Leeds).",
            "Focus on EPC compliance for older portfolios.",
        ],
    }
...
def _trigger_django_pdf_builder(**context):
    ...
    # POST to Django (Do not upload to Azure yet)
...
# ── DAG Definition ────────────────────────────────────────────────────────────
with DAG(
    dag_id="england_monthly_report",
    description="Monthly executive summary report for the England real estate market",
    schedule="0 0 1 * *",
    start_date=datetime(2025, 1, 1),
    catchup=False,
    default_args=DEFAULT_ARGS,
    tags=["england", "smartmove", "reports", "monthly"],
    max_active_runs=1,
) as dag:
