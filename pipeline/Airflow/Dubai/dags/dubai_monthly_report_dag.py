# ============================================================
# dags/dubai_monthly_report_dag.py
#
# DAG: dubai_monthly_report
#
# Schedule: 0 0 1 * * (midnight on the 1st of every month)
#
# Flow:
#   fetch_market_data          → Dummy SQL fetch (Dubai market KPIs)
#       │
...
_AIRFLOW_ROOT = Path(__file__).resolve().parents[2]  # e.g. pipeline/Airflow/Dubai/
...
def _fetch_market_data(**context):
    ...
    try:
        ...
        query = f"""
            ...
            FROM dbo.fact_transactions_dubai f
            JOIN dbo.dim_date_dubai d ON f.date_id = d.date_id
            JOIN dbo.dim_area_dubai a ON f.area_id = a.area_id
            JOIN dbo.dim_property_dubai p ON f.property_type_id = p.property_type_id
    ...
    smartmove_index = 78.4  # Retain dummy composite score for now
    ...
    if not df_6m.empty:
        for area, group_6m in df_6m.groupby('area_name_en'):
            ...
            # Price Volatility 6M (STDEV of monthly Avg Price per sqm)
            monthly_avg = group_6m.groupby(group_6m['transaction_date'].dt.to_period('M'))['meter_sale_price'].mean()
            ...
            mu = group_current['meter_sale_price'].mean()
            sigma = group_current['meter_sale_price'].std(ddof=1)
            if pd.isna(sigma):
                sigma = 0.0
                
            outliers = group_current[group_current['meter_sale_price'] > (mu + 2 * sigma)]
            outlier_ratio = len(outliers) / len(group_current) if len(group_current) > 0 else 0.0
            
            # Shock Index
            current_avg = group_current['meter_sale_price'].mean()
            avg_6m = group_6m['meter_sale_price'].mean()
    ...
    market_data = {
        ...
        "market_trends": [
            {
                "name": "Off-Plan Sales Volume",
                "direction": "Increasing",
                "magnitude": "+14.2% MoM",
            },
            {
                "name": "Villa Segment Premium",
                "direction": "Stable",
                "magnitude": "Holding at 23% above apartments",
            },
            {
                "name": "Mortgage Originations",
                "direction": "Increasing",
                "magnitude": "+6.1% MoM",
            },
            {
                "name": "Rental Yields (Avg.)",
                "direction": "Declining slightly",
                "magnitude": "-0.4pp to 6.8%",
            },
        ],
        "ai_recommendations": [
...
def _trigger_django_pdf_builder(**context):
    ...
    payload = {
        ...
        # Intentionally removing azure_container from this step for true parallel split
    }
...
# ── DAG Definition ────────────────────────────────────────────────────────────
with DAG(
    dag_id="dubai_monthly_report",
    description="Monthly executive summary report for the Dubai real estate market",
    schedule="0 0 1 * *",
    start_date=datetime(2026, 1, 1),
    catchup=False,
    default_args=DEFAULT_ARGS,
    tags=["dubai", "smartmove", "reports", "monthly"],
    max_active_runs=1,
) as dag:
